...
export async function sendMessage(message: IRabbitMessage): Promise<IRabbitMessage> {
    ...
    try {
        const exchange = await channel.assertExchange("levels", "fanout", { durable: false });
        if (channel.publish(exchange.exchange, "", Buffer.from(JSON.stringify(message)))) {
            console.log(">>>Published on fanout levels: ", message)
